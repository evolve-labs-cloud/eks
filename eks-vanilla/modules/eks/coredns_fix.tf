...
resource "aws_iam_role" "coredns_fix" {
  name_prefix        = format("%s-coredns-fix", var.prefix)
...
resource "aws_security_group" "coredns_fix" {
  name   = format("%s-coredns-fix", var.prefix)
  vpc_id = var.vpc_id
...
data "archive_file" "coredns_archive" {
  type        = "zip"
  source_dir  = "${path.module}/lambda/coredns"
  output_path = "${path.module}/lambda/coredns.zip"
}



resource "aws_lambda_function" "coredns_fix" {
  function_name = format("%s-coredns-fix", var.prefix)
  runtime       = "python3.13"
